...
namespace Trivia.Sessions
{
    ...
    public class GameSession
    {
        ...
        public GameSession(int numRounds, int numQuestionsPerRound, int pointValue, List<ActiveScorer> scorers)
        {
            NumRounds = numRounds;
            Rounds = new List<ScoringRound>();
            for (int i = 0; i < numRounds - 1; i++)
            {
                ScoringRound r = new ScoringRound(i, false, numQuestionsPerRound, pointValue);
                Rounds.Add(r);
            }
            Rounds.Add(new ScoringRound(numRounds - 1, true, 1, 0));  // add the bonus round with default params
            Scorers = scorers;
        }
